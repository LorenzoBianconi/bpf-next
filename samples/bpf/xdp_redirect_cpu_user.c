...
static void stats_print(struct stats_record *stats_rec,
			...
			char *prog_name, char *mprog_name)
{
	unsigned int nr_cpus = bpf_num_possible_cpus();
	...
	/* CPUMAP attached program */
	{
		...
		for (i = 0; i < nr_cpus; i++) {
			...
			if (xdp_pass > 0 || xdp_drop > 0 || xdp_redirect > 0)
				printf(fmt_k, "cpumap_kthread", i, xdp_pass, xdp_drop, xdp_redirect);
		}
		calc_xdp_pps(&rec->total, &prev->total, &xdp_pass, &xdp_drop,
			     &xdp_redirect, t);
		printf(fm2_k, "cpumap_kthread", "total", xdp_pass, xdp_drop, xdp_redirect);
	}
		       ...
		       bool stress_mode)
{
	...
	while (1) {
		swap(&prev, &record);
		stats_collect(record);
		stats_print(record, prev, prog_name, mprog_name);
		sleep(interval);
		if (stress_mode)
			stress_cpumap(value);
	}
