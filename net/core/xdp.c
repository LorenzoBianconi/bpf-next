...
static void hint_skb_record_rx_queue(struct sk_buff *skb,
...
static void hint_skb_set_hash(struct sk_buff *skb,
			      struct xdp_hints_common *hints)
{
...
static void hint_skb_checksum(struct sk_buff *skb,
			      struct xdp_hints_common *hints)
{
					   ...
					   struct net_device *dev)
{
	...
	if ((xdpf->flags & (XDP_FLAGS_HAS_HINTS|XDP_FLAGS_HINTS_COMPAT_COMMON))
	    && xdpf->metasize >= sizeof(*xdp_hints)) {
		xdp_hints = xdpf->data - sizeof(*xdp_hints);
		hint_skb_record_rx_queue(skb, xdp_hints);
		hint_skb_set_hash(skb, xdp_hints);
		hint_skb_checksum(skb, xdp_hints);
	}
