				     ...
				     struct bpf_prog *prog)
{
	...
	data = page_pool_dev_alloc_va(pp_ptr, &truesize);
	if (!data)
		return -ENOMEM;

	nskb = napi_build_skb(data, truesize);
	if (!nskb) {
		page_pool_free_va(pp_ptr, data, true);
		return -ENOMEM;
	}
	...
	for (i = 0; i < MAX_SKB_FRAGS && off < skb->len; i++) {
		...
		page = page_pool_dev_alloc(pp_ptr, &page_off, &truesize);
		if (!data) {
			consume_skb(nskb);
			return -ENOMEM;
		}
...
static int net_page_pool_alloc(int cpuid)
{
	...
	pp_ptr = page_pool_create(&page_pool_params);
	if (IS_ERR(pp_ptr)) {
		pp_ptr = NULL;
		return -ENOMEM;
	}

	page_pool_set_cpuid(pp_ptr, cpuid);
